...
class InsuranceMarkdownAnalyzer:
    ...
    def _extract_relevant_sections(self) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if 'Page' in line and ('of' in line or '/' in line or line.strip().startswith('Page')):
                if 'Page' in line:
                    try:
    ...
    def extract_with_openai(self) -> List[Dict[str, Any]]:
        ...
        # Count pages in the document (looking for page markers)
        pages = []
        lines = self.markdown_content.split('\n')
        for i, line in enumerate(lines):
            if 'Page' in line and ('of' in line or '/' in line or line.strip().startswith('Page')):
        ...
        if not pages:
            # Try to find page breaks by other patterns
            for i, line in enumerate(lines):
                if '---' in line or '___' in line or '===' in line:
                    pages.append(i)
        
        total_pages = len(pages) if pages else 1
        print(f"\nProcessing document with approximately {total_pages} page(s)...")
        ...
        if len(relevant_content) > 15000:
            print(f"Content too large ({len(relevant_content)} chars), prioritizing key sections...")
            ...
            print(f"Truncated to {len(relevant_content)} characters (20 sections)")
...
"""
        ...
        try:
            print(f"\nSending {len(relevant_content)} characters to OpenAI API...")
            ...
            try:
                ...
                if isinstance(result, dict) and 'buildings' in result:
                    ...
                    print(f"Successfully extracted {len(buildings)} building(s) from document")
                    print("=" * 80)
                    return buildings
                else:
